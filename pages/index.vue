...
const printAndProceed = async () => {
    ...
    const sendInvoice = async () => {
      await axios.post(`/invoice/${invoice.value.invoice_id}/send`)
        ...
        $swal.fire({
            title: 'Invoice was sent',
            text: `Invoice ${invoice.value.invoice_id} has been sent`,
            icon: 'success',
        })

        dialogDetail.show = false
    }
    const viewInvoice = async (item: any) => {
      const result = await axios.get(`invoice/${item.invoice_id}/pdf`, {
...
<template>
          ...
          <div class="select-none h-auto w-full text-center pt-3 pb-4 px-4">
            ...
            <div class="mb-3 text-blue-gray-700 px-3 pt-2 pb-3 rounded-lg bg-blue-gray-50">
              <div class="flex text-lg font-semibold">
                <div class="flex-grow text-left">Customer VATIN</div>
                <div class="flex text-right">
                  <input v-model="buyer_vatin" type="text" class="w-40 text-right bg-white shadow rounded-lg focus:bg-white focus:shadow-lg px-2 focus:outline-none">
                </div>
              </div>
              <!-- <div class="flex text-lg font-semibold">
                ...
                <div class="flex text-right">
                  ...
                  <input v-model="cash" type="text" class="w-28 text-right bg-white shadow rounded-lg focus:bg-white focus:shadow-lg px-2 focus:outline-none">
                </div>
              </div> -->
              <hr class="my-2">
              <!-- <div class="grid grid-cols-3 gap-2 mt-2">
                <template v-for="money in moneys">
                  <button @click="addCash(money)" class="bg-white rounded-lg shadow hover:shadow-lg focus:outline-none inline-block px-2 py-1 text-sm">+<span>{{ numberFormat(money) }}</span></button>
                </template>
              </div>
            </div>
            <div
    ...
    <div
      ...
      >
        <div>
          <button class="bg-cyan-500 fixed bottom-[20px] right-[150px] text-white text-lg px-4 py-3 rounded-2xl w-[200px] focus:outline-non" @click="sendInvoice()"> send</button>
        </div>
